...
**Input:** `the arguments supplied with the invocation` — a dataset path (e.g., `data/county_panel.csv`) or a description of the analysis goal (e.g., "regress wages on education with state fixed effects using CPS data").

---

## Constraints

- **Follow R code conventions** in `.codex/guidance/r-code-conventions.md`
- **Save all scripts** to `scripts/R/` with descriptive names
- **Save all outputs** (figures, tables, RDS) to `scripts/R/_outputs/`
- **Use `saveRDS()`** for every computed object — Markdown slides may need them
- **Use project theme** for all figures (check for custom theme in `.codex/guidance/`)
...
- `.codex/guidance/r-code-conventions.md` — [one-line summary of most relevant rule]
- `.codex/guidance/content-invariants.md` — [INV-9, INV-10, INV-11, INV-12 applicable]
...
For regressions, simulations, or bootstrap loops that take more than a couple of minutes, launch via Bash with `run_in_background: true` and then use Anthropic's running-process interface to stream R stdout into the conversation in real time. Pattern:
