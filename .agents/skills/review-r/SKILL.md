...
1. **Identify scripts to review:**
   - If `the arguments supplied with the invocation` is a specific `.R` filename: review that file only
   - If `the arguments supplied with the invocation` is `PresentationN`: review all R scripts matching that research presentation
   - If `the arguments supplied with the invocation` is `all`: review all R scripts in `scripts/R/` and `Figures/*/`

2. **For each script, launch the `r-reviewer` agent** with instructions to:
   - Follow the full protocol in the agent instructions
   - Read `.codex/guidance/r-code-conventions.md` for current standards
