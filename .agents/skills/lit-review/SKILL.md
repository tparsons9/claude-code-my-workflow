...
**Input:** `the arguments supplied with the invocation` — a topic, paper title, research question, or phenomenon to investigate.

---

## Steps

1. **Parse the topic** from `the arguments supplied with the invocation`. If a specific paper is named, use it as the anchor.

2. **Search for related work** using available tools:
   - Check `master_supporting_docs/supporting_papers/` for uploaded papers
   - Use `web search` to find recent publications (if available)
   - Use `web retrieval` to access working paper repositories (if available)
...
Before returning the draft literature review to the user, run the Post-Flight Verification protocol from [`.codex/guidance/post-flight-verification.md`](../../../.codex/guidance/post-flight-verification.md). Literature reviews are **very high** hallucination risk because web search can return plausible-sounding fabricated citations. CoVe catches this architecturally.
...
3. **Spawn `claim-verifier`** as a `claim-verifier` subagent and `context=fork`. Pass: the claims table, the verification questions, the source-material pointers (paper URLs, DOIs, `master_supporting_docs/` paths). **Do NOT pass the draft text itself** — the fresh-context independence is what makes CoVe work.
