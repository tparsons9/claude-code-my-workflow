...
1. **Identify files to audit:**
   - If `the arguments supplied with the invocation` starts with a filename: audit that file only.
   - If `the arguments supplied with the invocation` is `all`: audit all `.md`, `.tex`, `.md` files in `Slides/`, `Markdown/`, root, and `master_supporting_docs/`.
...
detector. See [`writing-with-ai.md`](../../../.codex/guidance/writing-with-ai.md).
