...
- `the first argument` (optional) — snippet name (without `.tex`). One of the filenames in `templates/tikz-snippets/`. If omitted, list the gallery and ask the user to pick.
- `the second argument` (optional) — output path. Defaults to `Figures/new_diagram.tex` — ask the user if this target exists so we don't clobber.
...
If `the first argument` is not one of these, or is omitted, ask the user which to use.

### Step 2: Copy the snippet to the output path

```bash
SRC="templates/tikz-snippets/the first argument.tex"
...
4. **Do not** add a bare `scale=X` to the tikzpicture options. Coordinates shrink, text does not — collisions follow. Allowed forms: `scale=X, every node/.style={scale=X}` or `scale=X, transform shape`. See [`tikz-prevention.md` P3](../../../.codex/guidance/tikz-prevention.md) for the full rule and `tikz-visual-quality.md` for the `scale=1.1` convention.
...
Spawn the `tikz-reviewer` agent as a `tikz-reviewer` subagent. Pass the `.tex` source and the compiled `.pdf` path. The reviewer is now required to cite the pass and formula from [`tikz-measurement.md`](../../../.codex/guidance/tikz-measurement.md) for every CRITICAL/MAJOR finding — vague reports are rejected.
...
### Step 7: Optional — convert to SVG for Markdown

If the user plans to use the diagram in Markdown slides (not just Beamer), convert the compiled PDF to SVG:

```bash
pdf2svg "${DST%.tex}.pdf" "${DST%.tex}.svg" 1
```

Snippet-based diagrams are single-page, so a single `.svg` with the same basename as `.tex` is correct here. This differs from `$extract-tikz`, which produces a multi-page PDF from a `Figures/PresentationN/extract_tikz.tex` master and names outputs `tikz_exact_00.svg, tikz_exact_01.svg, ...` (0-based filenames over 1-indexed PDF pages). If you need multi-page output for slide-by-slide reveals, use `$extract-tikz` instead.
...
- Reminder to `\input` or `\includegraphics` the diagram in the target Beamer/Markdown file
...
- [`.codex/guidance/tikz-prevention.md`](../../../.codex/guidance/tikz-prevention.md) — the P1–P6 authoring rules.
- [`.codex/guidance/tikz-measurement.md`](../../../.codex/guidance/tikz-measurement.md) — the six-pass protocol with formulas the reviewer cites.
- [`.codex/guidance/tikz-visual-quality.md`](../../../.codex/guidance/tikz-visual-quality.md) — general visual standards.
- [`.agents/skills/extract-tikz/SKILL.md`](../extract-tikz/SKILL.md) — for pulling TikZ out of an existing Beamer deck instead of creating new.
