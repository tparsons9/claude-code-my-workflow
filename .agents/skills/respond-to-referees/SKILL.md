...
- `the first argument` — path to the referee report
- `the second argument` — path to the revised manuscript

Supported formats and how to read them. In the commands below, `FILE` stands for the input path being converted — either `the first argument` (referee report) or `the second argument` (revised manuscript). Always use `mktemp` for the temp file (not a predictable `/tmp/...` name) so paths with spaces and concurrent runs don't collide, and so untrusted `FILE` paths can't clobber other temp files via symlink races.

| Format | How to extract text |
| --- | --- |
| `.tex` or `.md`, `.txt` | Read directly with the `Read` tool. |
...
3. **Spawn `claim-verifier`** as a `claim-verifier` subagent and `context=fork`. Hand it: the claims table, the verification questions, the path to the revised manuscript. Do NOT include the response draft.
