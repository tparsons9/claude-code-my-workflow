...
If the document cites prior literature in the rationale section (e.g., "Building on Hainmueller et al. 2014, we expect …"), invoke `$verify-claims` as a subagent to fact-check those citations. Pass the draft path and a list of explicit citations. The `claim-verifier` agent (forked context, never sees the draft) returns PASS / PARTIAL / FAIL per citation. Surface any FAIL/PARTIAL in the output summary.
...
- `.agents/skills/interview-me/SKILL.md` — produces the spec this skill consumes via `--input`.
- `.agents/skills/power-analysis/SKILL.md` — supplies the MDE / required-N + power paragraph for the sample-plan field (RCTs).
- `.agents/skills/verify-claims/SKILL.md` — Phase 5 invokes this for citation post-flight.
- `.codex/references/discipline-cards.md` — field defaults that drive `--style` selection.
- `.codex/guidance/replication-protocol.md` — preregistration is the *forward* commitment; replication-protocol is the *backward* contract.
...
**Citation post-flight fails with FAIL** — `claim-verifier` could not find a cited paper in web search / corpus. Either the citation is real but the verifier missed it (common with very recent / paywalled work — cite explicitly with URL), or the citation is hallucinated. Inspect manually before upload.
