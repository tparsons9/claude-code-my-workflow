...
**Input:** `the arguments supplied with the invocation` — a brief topic description or "start fresh" for an open-ended exploration.
...
3. **Spawn `claim-verifier`** as a `claim-verifier` subagent and `context=fork`. Hand it the claims + questions + source pointers (DOIs, arXiv links, `master_supporting_docs/` PDFs if the user provided any during the interview). Do NOT include the drafted spec.
