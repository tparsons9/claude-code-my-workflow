...
**Input:** `the arguments supplied with the invocation` — a topic (e.g., "minimum wage effects on employment"), a phenomenon (e.g., "why do firms cluster geographically?"), or a dataset description (e.g., "panel of US counties with pollution and health outcomes, 2000-2020").

---

## Steps

1. **Understand the input.** Read `the arguments supplied with the invocation` and any referenced files. Check `master_supporting_docs/` for related papers. Check `.codex/guidance/` for domain conventions.
...
3. **Tag each RQ with a likely paper type** (drawn from `methods-referee.md`):
   ...
   Use `.codex/references/discipline-cards.md` to bias the distribution by field (econ vs poli-sci default frequencies differ — e.g., poli-sci skews more toward `survey-experiment` and `formal-theory` than econ does).
...
Before returning the ideation report, run the Post-Flight Verification protocol from [`.codex/guidance/post-flight-verification.md`](../../../.codex/guidance/post-flight-verification.md). Research ideation is hallucination-prone in three specific ways:
...
3. **Spawn `claim-verifier`** as a `claim-verifier` subagent and `context=fork`. Hand it claims + questions + source pointers (web search allowed, NBER/SSRN URLs preferred, dataset codebooks preferred). Do NOT include the draft.
