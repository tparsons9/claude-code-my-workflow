...
> **AEA Data Editor / DCAS policy (applies to all AEA-imprint journals — AER, AEJ:*, JEL, JEP).** Acceptance is conditional on a replication package that clears the **AEA Data and Code Availability Standard** under the Data Editor: a complete deposit (openICPSR), a data availability statement, and code that reproduces every reported number. Run [`$replication-package`](../../.agents/skills/replication-package/SKILL.md) (which gates on [`$audit-reproducibility`](../../.agents/skills/audit-reproducibility/SKILL.md)) before submission; for restricted data, deposit cleared outputs + access instructions per [`confidential-data.md`](../guidance/confidential-data.md). Econometrica, ReStud, and the political-science journals below enforce comparable archives at acceptance.
...
- `.codex/agents/editor.md` — reads this file.
- `.codex/agents/domain-referee.md` — applies domain-referee adjustments.
- `.codex/agents/methods-referee.md` — applies methods-referee adjustments.
- `.agents/skills/review-paper/SKILL.md` — `--peer [journal]` mode entry point.
